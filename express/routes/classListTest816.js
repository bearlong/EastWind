...
router.get('/', async (req, res) => {
  try {
    ...
    if (course_category_id) {
      filteredCourses = filteredCourses.filter(course => course.course_category_id === parseInt(course_category_id))
    }

    if (price) {
      filteredCourses = filteredCourses.filter(course => course.price <= parseInt(price))
    }
